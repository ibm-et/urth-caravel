...
var px = (function () {
  ...
  var visualizations = window.visualizations = {};
  var slice;
  ...
  var Slice = function (data, dashboard, root) { //ALP
    var timer;
    var token = $('#' + data.token, root);
    var container_id = data.token + '_con';
    var selector = '#' + container_id;
    var container = $(selector, root);
    var slice_id = data.slice_id;
    var dttm = 0;
    var stopwatch = function () {
      dttm += 10;
      var num = dttm / 1000;
      $('#timer', root).text(num.toFixed(2) + " sec");
    };
    var qrystr = '';
    var always = function (data) {
      //Private f, runs after done and error
      clearInterval(timer);
      $('#timer', root).removeClass('btn-warning');
    };
    slice = {
      data: data,
      container: container,
      container_id: container_id,
      selector: d3.select(root).select(selector).node(),
      querystring: function () {
        ...
        } else if ($('#query', root).length === 0) {
          qrystr = parser.search;
        } else {
            qrystr = '?' + $('#query', root).serialize();
        }
        return qrystr;
      },
      ...
      jsonEndpoint: function () {
        var parser = document.createElement('a');
        parser.href = data.json_endpoint;
        var endpoint = 'http://192.168.99.100:8081' + parser.pathname + this.querystring(); // ALP: HACK
        endpoint += "&json=true";
        ...
        if (data !== undefined) {
            $("#query_container", root).html(data.query);
        }
        $('#timer', root).removeClass('btn-warning');
        $('#timer', root).addClass('btn-success');
        $('span.query', root).removeClass('disabled');
        $('#json', root).click(function () {
          window.location = data.json_endpoint;
        });
        $('#standalone', root).click(function () {
          window.location = data.standalone_endpoint;
        });
        $('#csv', root).click(function () {
          window.location = data.csv_endpoint;
        });
        $('.btn-group.results span,a', root).removeAttr('disabled');
        $('.query-and-save button', root).removeAttr('disabled');
        always(data);
      },
      error: function (msg) {
        token.find("img.loading").hide();
      ...
      render: function (force, root) {
        ...
        $('#timer', root).removeClass('btn-danger btn-success');
        $('#timer', root).addClass('btn-warning');
        this.viz.render();
      },
      resize: function () {
        token.find("img.loading").show();
  ...
  function registerViz(name) {
    ...
    //ALP
    if (visSource) {
        var visFactory = require('./visualizations/' + visSource); //ALP
